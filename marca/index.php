...
<script>
$(function() {
	jQuery("#marcasGrid").jqGrid({
			url:'ajaxListarMarcas.php',
			editurl:'marcaAction.php',
            datatype:'json',
            mtype:'GET',
            jsonReader:
				{'repeatitems':false},
            pager:'#marcasPagerGrid',
            rowNum:10,
            rowList:
			...
			caption: "Marcas",
            colModel:[
                {label:'Cód.',width:60,align:'center',name:'idMarca'},
				{label:'Marca',width:200,align:'center',name:'nomeMarca'}
            ] 
        });
	jQuery("#marcasGrid").jqGrid('navGrid', '#marcasPagerGrid', {del:false,add:false,edit:false,search:false,refresh:true} );
	
	//cadastro de categoria
	$("#btnCadastrar").click(function(){
		window.location = "marcaCad.php";				   
	})
	
	//edição de categoria
	$("#btnEditar").click(function(){
	    //Captura a linha selecionada na Grid
		var linhaSelecionada = jQuery("#marcasGrid").getGridParam('selrow');
			
		if(linhaSelecionada != null){
		    //Captura o ID na linha selecionada (célula da coluna 0 - zero)
		    var id = jQuery("#marcasGrid").getCell(linhaSelecionada,0);
			window.location = "marcaEdit.php?id="+id;				
		}else{
	...
	$("#btnDeletar").click(function(){
		...
		if(linhaSelecionada != null){
			...
			if (confirm("Confirma a exclusão?") == true){
				...
				$('#objetoQualquer').load('marcaAction.php?acao=excluir&idMarca='+id);

   			    jQuery("#marcaGrid").jqGrid('setGridParam',{url:'ajaxListarMarcas.php?txtNomeMarca=',page:1}).trigger('reloadGrid');
			}	
		}else{
			alert("Selecione um Registro");
		}			   
	...
	jQuery("#btnPesquisar").click(function(){
		var txtNome = $('#txtNomeMarca').val();	
		
		jQuery("#marcasGrid").jqGrid('setGridParam',{url:'ajaxListarMarcas.php?txtNomeMarca='+txtNome ,page:1}).trigger('reloadGrid');
		
	})
	
	jQuery("#btnLimpar").click(function(){	
		$('#txtNomeMarca').val('');			
		jQuery("#marcasGrid").jqGrid('setGridParam',{url:'ajaxListarMarcas.php' ,page:1}).trigger('reloadGrid');		
	})
});
</script>
</head>
...
<div id="botoes" style="padding:4px 4px 4px 4px; color:#666; font-size:12px; font-weight:bold;">
    ...
    <input type="text" id="txtNomeMarca" name="txtNomeMarca"/> 
    <input type="button" id="btnPesquisar" value="Pesquisar"/>  
    <input type="button" id="btnLimpar" value="Limpar"/>       
</div> 
<table id="marcasGrid" ></table>
<div id="marcasPagerGrid"></div>
